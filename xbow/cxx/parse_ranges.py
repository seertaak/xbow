...
from xbow.cxx.visitor import Visitor
...
def main():
    ...
    class print_ast(Visitor):
        ...
        def visit_generic(self, node):
            if 'tuple_algorithm.hpp' in str(node.location.file): return
            info = {
                'kind': node.kind,
                'usr': node.get_usr(),
                'spelling': node.spelling,
            ...
            print(indent("--", 4*self.depth*' '))
            print(indent("\n".join([f"{f}:{v}" for f, v in info.items()]), 4*self.depth*' '))


    class print_views(Visitor):
        def __init__(self, tu):
            self.tu = tu

        def visit_StructDecl(self: 'print_views', node) -> None:
            if '_fwd' in str(node.location.file): return
            if node.spelling.endswith("_fn") and 'single' in node.spelling:
                printer = print_ast(node)
                printer.visit(node.get_definition())
